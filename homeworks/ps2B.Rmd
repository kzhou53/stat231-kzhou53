...
pitching_clean
```
There are 10 pitchers that have had more than 300 wins and 3000 strike outs over their career.
...
The player with the most accumulated strikeouts was ryanno01 with a total of 5714 strikeouts over his career. The most strikeouts he got in a season was 383 in 1973.
...
FakeDataLong <- FakeDataLong %>%
  group_by(grp, sex) %>%
  summarise(mean_L = meanL, mean_R = meanR, sd_L = sdL, sd_R = sdR) %>%
  gather(key = mean_LR, value = mean, mean_L, mean_R) %>%
  gather(key = sd_LR, value = sd, sd_L, sd_R)


  #spread(key = sex, value = mean_L, fill = 0) 
