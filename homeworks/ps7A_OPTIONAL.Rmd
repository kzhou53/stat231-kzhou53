...
> ANSWER: 726 cities in cluster 1. Cluster 1 represents the first group of largest cities that are most closely related to each other. This happens to be around South East Asia.
...
> ANSWER: Now cluster 1 has 1466 cities and represents cities from Europe and Africa that are closely related.
...
BigCities_km2 <- BigCities %>%
  mutate(cluster2 = as.character(km_out2$cluster))

ggplot(data = BigCities_km2, aes(x = longitude, y = latitude)) +
  geom_point(aes(color = cluster2), alpha = 0.5) +
  geom_point(data = as.data.frame(km_out2$centers), shape = "X", size = 3)
...
> ANSWER: Now cluster 1 is only 593 cities and it represents the cities in India which are closely related.    
...
BigCities_km3 <- BigCities %>%
  mutate(cluster3 = as.character(km_out3$cluster))

ggplot(data = BigCities_km3, aes(x = longitude, y = latitude)) +
  geom_point(aes(color = cluster3), alpha = 0.5) +
  geom_point(data = as.data.frame(km_out3$centers), shape = "X", size = 3)
