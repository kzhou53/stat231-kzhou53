...
tweet_bigrams %>%
  ...
  labs(y = "Number of instances"
       , title="The most common bigrams in Trump's tweets by source") +
...
> ANSWER: The proportion of words with angry sentiment is higher from both sources than the proportion with joyful sentiment, but not by a large amount. Compared to the proportion of angry and joyful words, there are higher proportions of negative and positive words across both sources. The proportion of positive words from both sources is slightly higher than the proportion of negative words, but similar to the pattern in the first analysis, not higher by much. 

```{r}
nrc_lexicon <- get_sentiments("nrc")

iphone_sentiment <- inner_join(iphone_word, nrc_lexicon, by = "word")
iphone_tot <- nrow(iphone_word)
numiphone_anger <- length(iphone_sentiment$sentiment[iphone_sentiment$sentiment 
                                                     == "anger"])
numiphone_joy <- length(iphone_sentiment$sentiment[iphone_sentiment$sentiment 
                                                   == "joy"])

android_sentiment <- inner_join(android_word, nrc_lexicon, by = "word")
andr_tot <- nrow(android_word)
numandr_anger <- length(android_sentiment$sentiment[android_sentiment$sentiment 
                                                       == "anger"])
numandr_joy <- length(android_sentiment$sentiment[android_sentiment$sentiment 
                                                  == "joy"])
prop_sentiment <- data.frame(
  numiphone_anger,
  numiphone_joy,
  iphone_tot,
  numandr_anger,
  numandr_joy,
  andr_tot) %>%
  mutate(prop_iphone_anger = (numiphone_anger/iphone_tot),
         prop_iphone_joy = (numiphone_joy/iphone_tot),
         prop_andr_anger = (numandr_anger/andr_tot),
         prop_andr_joy = (numandr_joy/andr_tot)
         )
```
...
iphone_sentiment <- inner_join(iphone_word, nrc_lexicon, by = "word")
iphone_tot <- nrow(iphone_word)
numiphone_pos <- length(iphone_sentiment$sentiment[iphone_sentiment$sentiment 
                                                     == "positive"])
numiphone_neg <- length(iphone_sentiment$sentiment[iphone_sentiment$sentiment 
                                                   == "negative"])

android_sentiment <- inner_join(android_word, nrc_lexicon, by = "word")
andr_tot <- nrow(android_word)
numandr_pos <- length(android_sentiment$sentiment[android_sentiment$sentiment 
                                                       == "positive"])
numandr_neg <- length(android_sentiment$sentiment[android_sentiment$sentiment 
                                                  == "negative"])
prop_sentiment2 <- data.frame(
  numiphone_pos,
  numiphone_neg,
  iphone_tot,
  numandr_pos,
  numandr_neg,
  andr_tot) %>%
  mutate(prop_iphone_pos = (numiphone_pos/iphone_tot),
         prop_iphone_neg = (numiphone_neg/iphone_tot),
         prop_andr_pos = (numandr_pos/andr_tot),
         prop_andr_neg = (numandr_neg/andr_tot)
         )
```
