...
> RESPONSE:
<<<<<<< HEAD
There are 1846 rows and 3 columns for the variables. These variables are dataset, x-values, and y-values.
=======
There are 1846 rows and 3 variables. These variables are dataset, x, and y.
>>>>>>> b3598b0ce3f98bd12af12267bababbb1db456778
...
dino_data %>%
  summarize(r = cor(x, y))
```
The correlation coefficient between x and y in the dino dataset is -0.064. This means that the relationship between x and y is an inverse relationship. So as x increases, y will decrease and as x decreases, y will increase. However, since the  coefficient is so small, this relationship is not a strong one.  
...
The relationship between x and y forms a dinosaur shape however, in terms of regularity, ie, linear, exponential, etc. the relationship is very weak. 
...
star_data %>%
  summarize(r = cor(x, y))
```
The correlation coefficient for the star dataset between x and y is almost the same as the dino dataset correlation coefficient with a vlue of -0.063.
...
ggplot(data = star_data, mapping = aes(x = x, y = y)) +
  geom_point()
```
This plot is different from the dino dataset plot. Instead of a dino, it forms a star shape.
...
circle_data %>%
  summarize(r = cor(x, y))
```
As with the previous two datasets, the correlation coefficient is almost the same for this data set with a value of -0.068.
...
ggplot(data = circle_data, mapping = aes(x = x, y = y)) +
  geom_point()
```
The plot is different again and is now a circle/oval shape.
...
> RESPONSE:
The correlation coefficient is not an appropriate summary for these data sets because while their values are very similar across all datasets, the actual plot and distribution of the data is very different.
