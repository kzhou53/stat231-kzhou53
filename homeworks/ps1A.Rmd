...
```{r}
g + geom_text(aes(label = country, color = net_users), size = 3) + labs(x = "% of GDP spent on education", y = "Gross Domestic Product (GDP)") + theme(legend.position = "top") + scale_y_continuous(
...
p
```
The reorder() function in the above code reorganizes the data by its y-axis values, from smallest to largest. Without the function present, the data is plotted by ascending x-axis values.
...
p
```
The theme line essentially scaled the x-axis labels so that they were readable. It sized down the text so that they were spaced out and all desired labels were still there.The theme line also rotated the values 90 degrees counterclockwise.
...
I am not sure I uderstand the "make_babynames_dist()" function provided by the mdsr package. How do we know what variables and filters it adds? Was it preset specificlaly to 2014 or where in our code did we get that from and can we change it?

In the "tribble()" function, I don't understand what the year, number of people, and label correspond to. For example, "the median living Joseph age is ..." label is not pointing to 2003. Are 2003, 40 the location for where the text will be centered on the plot?
