...
> ANSWER: There are currently 12 tables on the page.
...
> ANSWER: Genzebe Dibaba holds the indoor one-mile record for IAAF women and her time is 4:13.31.
...
# create Type column
Type1 <- c("Indoor")
women_indoor <- data.frame(women_indoor, Type1)

Type2 <- c("Outdoor")
women_outdoor <- data.frame(women_outdoor, Type2)

women <- bind_rows(women_indoor, women_outdoor) %>%
  mutate(Type = coalesce(Type1, Type2)) %>%
