...
> ANSWER: Looking at the box plot, it appears that the tenure length is varies much more in the 1800 and 1900s and the medians are much higher during these times. Additionally, the median tenure length is shortes during the 1700s. The tenure length variation in the 1800 and 1900s is most likely due to the large amount of data available for both time periods compared to much fewer entries for the 1700s and 2000s. 

```{r}
justices <- read_csv("/Users/kimberlyzhou/git/stat231-kzhou53/homeworks/justices.csv")
...
```{r}
justices_small <- justices2 %>%
  select(justice_2, position, tenure_length, date_confirmed) %>%
  mutate(date_conf = as.character(date_confirmed))

# group justices by the year they were confirmed
justices_year <- justices_small %>%
  mutate(year = case_when(str_detect(date_conf, "^17") ~ "1700s", 
                          str_detect(date_conf, "^18") ~ "1800s",
                          str_detect(date_conf, "^19") ~ "1900s",
                          str_detect(date_conf, "^20") ~ "2000s",
                          TRUE ~ date_conf))

ggplot(data = justices_year, mapping = aes(x = year, y = tenure_length)) +
  geom_boxplot() +
  labs(
    x = "Year Range",
    y = "Tenure Length"
...
quotes_dat <- data.frame(quote = quote, stringsAsFactors = FALSE) %>%
  filter(quote != "\n") %>%
  mutate(person = person
         , together = paste('"', as.character(quote), '" --'
                          , as.character(person), sep=""))
...
```{r, linewidth = 60}
quotes_dat <- read_csv("/Users/kimberlyzhou/git/stat231-kzhou53/homeworks/quotes.csv")
