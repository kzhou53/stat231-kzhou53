...
```
Adding "shape = 2" changes the points on the graphic to an empty triangle. When we change it to "shape = "*"" the shape becomes an asterisk and "size = 5" just changes the size fo the symbol. If we add the aes function to geom point and then add shape with the same label as color, we have different shapes for different countries and a single legend that encompasses both these visual cues.
...
```
Using the exact same process as the previous question but with lty instead of shape, we can give each country a different line type and clean up the legend.
...
```
Adding the color brewer function by itself changes teh colors to shades of blue. 
