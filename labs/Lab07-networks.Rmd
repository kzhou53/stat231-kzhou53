...
> ANSWER:  str_detect for character names, count words between character names, filter for entries that are <15 from one another
...
got_connect <- graph_from_data_frame(got, directed = FALSE)
...
> ANSWER:  352 edges; 0.062 is the proportion of realized edges so the density is sparse.
...
> ANSWER: The graph is connected means that no node without an edge.

```{r}
is.connected(got_connect)
```


4.  Use the code below to compute the diameter of the network.  Interpret the value.

> ANSWER:  The diameter of the network is 6. It would take at most 6 connections to get between the two least connected characters.

```{r}
diameter(got_connect, directed = FALSE)
```
...
```{r}
V(got_connect)$degree <- igraph::degree(got_connect)
V(got_connect)$wtdegree <- strength(got_connect
                                , weights=E(got_connect)$Weight)

stats <- data_frame(name = V(got_connect)$name, 
                    degree = V(got_connect)$degree, 
                    wtdegree = V(got_connect)$wtdegree )
```
...
```{r}
igraph::degree(got_connect, mode = "in")
deg <- degree(got_connect, mode = "in")
sort(deg, decreasing = T)[1:5]

deg2 <- strength(got_connect, weights = E(got_connect)$Weight
         , mode = "total")
sort(deg2, decreasing = T)[1:5]
```
