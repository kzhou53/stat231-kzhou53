...
```{r}
coverage <- read_csv("/Users/kimberlyzhou/git/stat231-kzhou53/data/healthcare_coverage.txt")
spending <- read_csv("/Users/kimberlyzhou/git/stat231-kzhou53/data/healthcare_spending.txt")
```
...
spending_cleaned <- spending %>%
  select(Location, starts_with("2013"), starts_with("2014"))

healthcare_joined <- coverage_cleaned %>%
  inner_join(spending_cleaned, by = c("Location" = "Location"))

```
Using the View function in the console, we can see the entire data set for coverage, spending, and state. In coverage, the years span from 2013-2016. In spending, the years are from 1991-2014.
...
In order to create the scatterplot in ggplot we will to mutate the spending and employer coverage data so that they are in log form. We also need to create year as a variable since we are coloring according to this criteria. Each row in the dataset will represent a state and its corresponding data, but note that each state will have 2 rows: one for 2013 data and one for 2014 data.

What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: 

First we need to join together the two datasets, which we did in the warm-up. Then we should use the select function to select for the desired columns and we want to narrow the table so that data from different years is separated into different rows using the gather function. We will then need to mutate spending and employer coverage data by taking the logs of both. Additionally, in geom_point we will need to use the aes function to color by the variable year.
...
```{r}
healthcare_joined %>%
  select(Location, `2013__Employer`, `2013__Total`, `2014__Employer`, `2014__Total`) %>%
  mutate(log_2013Emp = log(`2013__Employer`), log_2014Emp = log(`2014__Employer`)) %>%
  gather(key = year, value = total, "2013__Total", "2014__Total") %>%
  gather(key = year, value = employee, "2013__Employer", "2014__Employer")
